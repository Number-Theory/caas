									...
									<div class="input-group">
										<select id="templateType" name="templateType"
											...
											<option value="0" hassubinfo="true"
												<c:if test="${returnMap.templateType == 0}">selected="selected"</c:if>>语音通知文字模板</option>
										</select>
									</div>
								</div>
							</div>
							...
							<div class="form-group">
								...
								<div class="col-sm-3">
									<textarea id="templateContent" name="templateContent"
										style="height: 100px;" minlength="1" type="text"
										class="form-control" required="" aria-required="true">${returnMap.templateContent }</textarea>
								</div>
							</div>
								...
								<div class="col-sm-8">
									<div class="input-group">
										<select id="status" name="status" data-placeholder="模板状态"
											...
											<option value="0" hassubinfo="true"
												<c:if test="${returnMap.status == 0}">selected="selected"</c:if>>审核通过</option>
											<option value="1" hassubinfo="true"
												<c:if test="${returnMap.status == 1}">selected="selected"</c:if>>待审核</option>
											<option value="2" hassubinfo="true"
												<c:if test="${returnMap.status == 2}">selected="selected"</c:if>>审核不通过</option>
											<option value="3" hassubinfo="true"
												<c:if test="${returnMap.status == 3}">selected="selected"</c:if>>已删除</option>
										</select>
									</div>
								</div>
							</div>
							...
							<div class="form-group">
								<div class="col-sm-6 col-sm-offset-5">
									<button class="btn btn-primary" type="button"
										onclick="editTemplate()">提交添加</button>
									<p id="editText"></p>
								</div>
							</div>
						</form>
