...
<body>
	<div class="wrap container-fluid about-certify">
			...
			<div class="content">
				<div class="box-fff clearfix">
					<div class="certify-edit">
						<h2 class="crumbs-menu"><span class="crumbs" onClick="history.back();"></span>企业开发者认证</h2>
						
						<!-- 组织 -->
						<div class="cert_items" id="cert_items_company">
							<form method="post" action="<%=path %>/user/oAuthCompany" enctype="multipart/form-data" name="companyForm" id="companyForm" >
							...
							<div class="group clearfix">
								<label>公司名称</label>
								<input type="text"  value="${oauth.userName}" id="company_name" name="oauth.userName" 
								onkeyup="if(this.value.length>60)this.value=this.value.subString(0,60)" onblur="oauth_frm_company.companyName();companyNameExist()"/>
								<span class="tips tips1">认证成功后将默认作为发票开票抬头</span>
								<p class="error" style='display:none' id="companyNameSpanError">错误：请输入正确的公司名称</p>
							</div>
							<div class="group clearfix">
								<label>公司注册地址</label>
								<input type="text" value="${oauth.webSite }" id="company_address" onblur="oauth_frm_company.address()" name="oauth.webSite" class="cert_num" onkeyup="if(this.value.length>200)this.value=this.value.subString(0,60)"/>
							...
							<div class="add_items" id="add_items1">
			              		<div class="group clearfix">
									<label>营业执照号</label>
									<input type="text" value="${oauth.enterpriseMaterialId }" id="bs_num" name="oauth.enterpriseMaterialId" onblur="oauth_frm_company.bsNum();idNumExists(this.value,'BSNUM','${oauth.enterpriseMaterialId }')" maxlength="20"/>
									<span class="tips tips1">20位数字或字母以内</span>
									<p class="error" style='display:none' id="bsnumSpanError">错误：请输入正确的营业执照号</p>
								</div>
								<div class="group clearfix">
									<label>三证合一照</label>
									<div class="rz-img ft-l clearfix"> 
										<input type="file" class="file_1" name="bsnumFile" onchange="verifyPic(this,3)"  id="bsnumFile"/>
				            ...
				            <s:if test="data.cerType==null">
				            	<input type="hidden" id="cerType" name="userPic.cerType" value="1"/>
				            </s:if>
				            <s:else>
				            	<input type="hidden" id="cerType" name="userPic.cerType" value="${data.cerType}"/>
				            </s:else>
				            
				            <input type="hidden" id="existsCompanyName" value="0">
				            
							</form>
						</div>
						
						<div class="div-btn cp-btn">
							<a href="javascript:void(0)" id="company_oauth_confirm_btn" class="sure btn-style4">提交审核</a>
						</div>
					</div>					
				</div>
			</div>
		...
		<script type="text/javascript">
		 $(function(){
			var cerType = "${data.cerType}";
			// 如果是普通验证并且表单数据为空，则将默认选项修改为4证
			if (cerType == "1" && $("#company_name").val() == "") {
				setCerType(1);
			}
		 });
		 
		 //企业认证类型
		 function setCerType(type){
			 $("#cerType").val(type);
			 $("#add_items"+type).show();
		 }
		</script>
